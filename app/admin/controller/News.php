...
use Overtrue\Socialite\Providers\FacebookProvider;
use think\Db;
...
class News extends Base
{
	...
	public function news_list()
	{
		...
		$news=$news_model->alias("a")->field('a.*,b.*,c.menu_name')
				...
				->where($map)->where($where)->order('news_time desc')->field('a.n_id')->paginate(config('paginate.list_rows'),false,['query'=>get_query()]);
		$show = $news->render();
...
//            exit;
	...
	public function news_add()
	{
	    ...
	    $menu_text=menu_text($this->lang);
	    $menu_list = Db::name('member_list')->field('member_list_id,member_list_nickname')->select();
	    $address = Db::query("SELECT c.id,c.menu_name FROM p_menu AS a JOIN p_menu AS b ON a.id=b.parentid JOIN p_menu AS c ON b.id=c.parentid WHERE a.id=23");
	...
	public function news_back()
	{
		...
		$news=$news_model->alias("a")->field('a.*,b.*,c.menu_name')
				...
				->where($where)->order('news_time desc')->field("a.n_id")->paginate(config('paginate.list_rows'),false,['query'=>get_query()]);
		$show = $news->render();
		$show=preg_replace("(<a[^>]*page[=|/](\d+).+?>(.+?)<\/a>)","<a href='javascript:ajax_page($1);'>$2</a>",$show);
		$this->assign('page',$show);
		//文章属性数据
...
//		exit;
    ...
    public function news_biaoqian(){
	    $biaoqian = Db::name('pag')->paginate(20);
//	    echo "<pre>";
//	    print_r($biaoqian);
//	    exit;
    ...
    public function news_open(){
        $id = input('id');
        $open = Db::name('pag')->where(['id'=>$id])->field('pag_open')->find();
        if($open['pag_open'] == 0){
            $su = Db::name('pag')->where(['id'=>$id])->update(['pag_open'=>1]);
            if($su == true){
                $this->success('推荐');
            }else{
                $this->error();
            ...
            $su = Db::name('pag')->where(['id'=>$id])->update(['pag_open'=>0]);
            if($su == true){
                $this->success('不推荐');
            }else{
                $this->error();
    ...
    public function pag_exit(){
        $id = input('id');
        $open = Db::name('pag')->where(['id'=>$id])->field('id,pag_quanname,p_pag_time,pag_jian,pag_img')->find();
        $this->assign('open',$open);
        return $this->view->fetch("news_pag_edit");
    }
    public function news_pagedit(){
        $news_pagedit = input('post.');
        unset($news_pagedit['file']);
        $news_pagedit['p_pag_time'] = strtotime( $news_pagedit['p_pag_time']);
        $id = $news_pagedit['n_id'];
        unset($news_pagedit['n_id']);
        $su = Db::name('pag')->where(['id'=>$id])->update($news_pagedit);
        if($su == true){
            $this->success('ok');
        }else{
            $this->error('no');
    ...
    public function news_nominate(){
        $id = input('id');
        $news_nominate = Db::query("SELECT nominate FROM p_news WHERE n_id=$id");
       foreach ($news_nominate as $k=>$v){
           $news_nominate= $v['nominate'];
       }
       if($news_nominate == 0){

         $succeed = Db::execute("UPDATE p_news SET nominate=1 WHERE n_id=$id");
           if($succeed == true){
               $this->success('推荐');
           }else{
               $this->error('GG');
           ...
           $succeed = Db::execute("UPDATE p_news SET nominate=0 WHERE n_id=$id");
           if($succeed == true){
               $this->success('不推荐');
           }else{
               $this->error('推荐');
