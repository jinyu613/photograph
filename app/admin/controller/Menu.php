...
//		exit;
	...
	public function news_menu_edit()
	{
        ...
        if(!empty($zone)){
            $region = Db::query("SELECT member_list_id,member_list_nickname FROM p_member_list WHERE area_id=$id");
            $this->assign('region',$region);
        }
        if($menu['groom_id'] == null){
            $menu['groom_id'] = Db::query("SELECT member_list_id,member_list_nickname FROM p_member_list");
        }else{
            $menu['groom_id'] = Db::query("SELECT member_list_id,member_list_nickname FROM p_member_list WHERE member_list_id IN(".$menu['groom_id'].")");
        }
	...
	public function news_menu_runedit()
	{
		...
		if (!request()->isAjax()){
            ...
            $groom_id = input('groom_id/a');
            if(!empty($groom_id)){
                $groom_id = implode(',',$groom_id);
            }else{
                $groom_id = "";
            }
			$data=array(
			    'groom_id'=>$groom_id,
			    'stationmaster_id'=>input('stationmaster_id'),
				'id'=>input('id'),
				'menu_name'=>input('menu_name'),
				'menu_enname'=>input('menu_enname'),
				'menu_type'=>input('menu_type'),
	...
	public function news_remen(){
	    $id = input('id');
	    $su = Db::name('menu')->where([
	        'id'=>$id
        ])->field('menu_remen')->find();
	    if($su['menu_remen'] == 0){
