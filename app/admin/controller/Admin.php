...
class Admin extends Base
{
	...
	public function classification(){
	    $classification = Db::query("SELECT * FROM p_assortment");
	    $this->assign('classification',$classification);
	    return view();
    }
    public function addfelei(){
	    return view();
    }
    public function admin_assortment(){
        $name = input('assortment_name');
        if($name !=null){
            $name_f = Db::query("SELECT * FROM p_assortment WHERE assortment_name=\"".$name."\"");
            if($name_f ==true){
                $this->error('标签已存在');
            }else{
                $su = Db::execute("INSERT INTO p_assortment (assortment_name) VALUES ("."\"".$name."\")");
                if($su != false){
                    $this->success('ok');
                }else{
                    $this->error('no');
